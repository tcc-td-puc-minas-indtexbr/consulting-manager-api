...
class ConsultingManagerServiceTestCase(BaseFunctionalTestCase):
    ...
    def setUpClass(cls):
        ...
        if cls.EXECUTE_FIXTURE:
            ...
            file_name = ROOT_DIR + '/tests/datasets/nosql/create.table.consulting-manager-dynamodb-ConsultingTable.json'
            DynamoDBHelper.create_table(connection=get_connection(),
                                        table_name=table_name,
                                        file_name=file_name)
            logger.info(f'table {table_name} created')

            file_name = ROOT_DIR + '/tests/datasets/seeders/seeder.consulting-manager-dynamodb-ConsultingTable.json'
    ...
    def test_list(self, request):
        ...
        service = ConsultingManagerService(logger=self.logger, config=self.CONFIG, connection=get_connection())
